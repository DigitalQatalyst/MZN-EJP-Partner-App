...
export const CommunityCard: React.FC<CommunityCardProps> = ({
  ...
  return <BaseCard onClick={onQuickView} data-id={dataId} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
      {/* Large Image Section */}
      <div className="relative w-full h-48 bg-gray-200 overflow-hidden flex-shrink-0">
        {item.imageUrl ? <img src={item.imageUrl} alt={`${item.name} community`} className={`w-full h-full object-cover transition-transform duration-300 ${isHovered ? 'scale-105' : 'scale-100'}`} /> : <div className="w-full h-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center">
